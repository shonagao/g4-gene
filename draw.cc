char PARTICLE[30][20] = {"gamma", "e-", "e+", "mu-", "mu+",
                         ...
                         "triton", "alpha", "Li7", "", ""
                         "", "", "", "", ""};
char PROCESS[20][100]= {"mother", "eBrem", "eIoni", "conv", "positronNuclear",
                        "Decay", "photonNuclear", "electronNuclear", "compt", "",
                        "pi-Inelastic", "pi+Inelastic", "protonInelastic", "neutronInelastic", "dInelastic",
                        "hBertini", "", "", "", ""};
...
void draw(){
  ...
  for(int i=0;i<30;i++){
    ...
    h_p_theta[i] = new TH2D(Form("h_p_theta_%d",i),Form("h_p_theta_%d",i),100,0.,60.,100,0,4000.);
    SetTH2(h_p_theta[i],Form("Mom v.s #theta (%s)",PARTICLE[i]),"#theta (degree)","Momentum (MeV/c)");
  }

  int nhit, pid[10], trackid[10];
  double theta[10], p[10], ptheta[10];
